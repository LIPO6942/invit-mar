...
function downloadSouvenirImage() {
  ...
  if (typeof html2canvas === 'undefined') {
    alert(isFr ? "La bibliotheque d'image est en cours de chargement, veuillez reessayer." : 'Ø¬Ø§Ø±ÙŠ ØªØ­Ù…ÙŠÙ„ Ù…ÙƒØªØ¨Ø© Ø§Ù„ØµÙˆØ±ØŒ ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ø¨Ø¹Ø¯ Ø«ÙˆØ§Ù†Ù.');
    return;
  }

  const data = _getSouvenirData();
  const { guestTitle, guestName, groom, bride, dateStr, dayNum, monthName, year } = data;
  const isFr2 = typeof _currentLang !== 'undefined' && _currentLang === 'fr';
  const isRtl = !isFr2;
  const dir = isRtl ? 'rtl' : 'ltr';

  // Floral corner SVG
  const corner = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 90" width="90" height="90">
    <g stroke="#c9a84c" stroke-width="1.1" fill="none" stroke-linecap="round" opacity="0.55">
      ...
      <ellipse cx="54" cy="10" rx="6" ry="10" transform="rotate(-10,54,10)"/>
      <ellipse cx="54" cy="10" rx="6" ry="10" transform="rotate(30,54,10)"/>
      <ellipse cx="54" cy="10" rx="6" ry="10" transform="rotate(70,54,10)"/>
      <ellipse cx="54" cy="10" rx="6" ry="10" transform="rotate(110,54,10)"/>
      <circle cx="54" cy="10" r="5" fill="#f5e190" opacity="0.9" stroke="none"/>
      <circle cx="54" cy="10" r="2.5" fill="#8a5d00" stroke="none"/>
      <ellipse cx="38" cy="28" rx="4" ry="6" transform="rotate(-25,38,28)"/>
      <circle cx="38" cy="22" r="3" fill="#f5e190" opacity="0.75" stroke="none"/>
      <circle cx="15" cy="72" r="1.5" fill="#c9a84c" stroke="none"/>
      <circle cx="42" cy="48" r="1" fill="#f5e190" stroke="none"/>
      <circle cx="62" cy="18" r="1.5" fill="#c9a84c" stroke="none"/>
    </g>
  </svg>`;
  ...
  // Gold bokeh SVG
  const bokeh = `<svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;" xmlns="http://www.w3.org/2000/svg">
  ...
  canvas.innerHTML = bokeh + `
    <div style="
      position:relative;
      background:linear-gradient(145deg,#fffef8 0%,#f8f4e8 100%);
      border-radius:6px;
      padding:14px 14px 48px;
      box-shadow:0 0 0 1px rgba(180,140,60,0.3),0 4px 8px rgba(0,0,0,0.35),0 16px 40px rgba(0,0,0,0.5);
      overflow:hidden;
    ">
      <!-- Floral corners -->
      ...
      <div style="position:absolute;bottom:48px;${leftPad}:0;width:90px;height:90px;overflow:hidden;transform:scaleY(-1);">${corner}</div>
      <div style="position:absolute;bottom:48px;${rightPad}:0;width:90px;height:90px;overflow:hidden;transform:scale(-1,-1);">${corner}</div>

      <!-- Inner card -->
      <div style="position:relative;background:linear-gradient(160deg,#fdf8ee 0%,#f7eed9 40%,#eedfc0 100%);border:2px solid rgba(201,168,76,0.6);border-radius:4px;overflow:hidden;box-shadow:inset 0 0 20px rgba(201,168,76,0.08);">
        ...
        <div style="position:relative;height:32px;display:flex;align-items:center;justify-content:center;">
          ...
          <span style="position:relative;font-family:cursive;font-size:1rem;color:#1a1000;z-index:2;">
            ${isFr2 ? 'âœ¦ Invitation au Mariage âœ¦' : 'âœ¦ Ø¯Ø¹ÙˆØ© Ø§Ù„Ø²ÙØ§Ù âœ¦'}
          </span>
        </div>
        ...
        <div style="padding:22px 30px 18px;text-align:center;direction:${dir};">
          ...
          <div style="background:linear-gradient(135deg,rgba(201,168,76,0.16),rgba(201,168,76,0.04));border:1.5px solid rgba(201,168,76,0.45);border-radius:12px;padding:14px 18px;margin-bottom:16px;">
            <div style="font-size:0.65rem;color:rgba(201,168,76,0.6);letter-spacing:4px;margin-bottom:5px;">âœ¦ âœ¦ âœ¦</div>
            ...
            <div style="font-size:0.65rem;color:rgba(201,168,76,0.6);letter-spacing:3px;margin-top:5px;">â€” âœ¦ â€”</div>
          </div>

          <!-- Seal divider -->
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
            <div style="flex:1;height:1px;background:linear-gradient(90deg,transparent,rgba(201,168,76,0.5));"></div>
            <div style="width:40px;height:40px;border-radius:50%;background:radial-gradient(circle at 38% 35%,#f5e190 0%,#d4a830 25%,#b8860b 50%,#7a5300 80%,#3d2600 100%);border:1.5px solid #c9a84c;box-shadow:0 0 0 3px rgba(201,168,76,0.2),0 5px 14px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;font-size:1rem;color:rgba(255,252,220,0.9);">âœ¦</div>
            <div style="flex:1;height:1px;background:linear-gradient(90deg,rgba(201,168,76,0.5),transparent);"></div>
          </div>

          <div style="font-size:1rem;color:#6b4d12;line-height:1.7;margin-bottom:3px;">Ø¨Ø§Ø±Ùƒ Ø§Ù„Ù„Ù‡ Ù„Ù‡Ù…Ø§ ÙˆØ¨Ø§Ø±Ùƒ Ø¹Ù„ÙŠÙ‡Ù…Ø§ ÙˆØ¬Ù…Ø¹ Ø¨ÙŠÙ†Ù‡Ù…Ø§ ÙÙŠ Ø®ÙŠØ±</div>
          <div style="color:#c9a84c;letter-spacing:4px;font-size:0.65rem;margin:6px 0;">âœ¦ âœ¦ âœ¦</div>
          <div style="font-size:0.88rem;color:#7a5c25;margin-bottom:10px;">
            ${isFr2 ? "Les familles vous invitent au mariage de leurs enfants" : "Ø¨Ø¯Ø¹ÙˆØªÙƒÙ… Ù„Ø­Ø¶ÙˆØ± Ø­ÙÙ„ Ø²ÙØ§Ù Ù†Ø¬Ù„ÙŠÙ‡Ù…Ø§"}
          </div>
          ...
          <div style="background:rgba(255,252,240,0.85);border:1px solid rgba(201,168,76,0.3);border-radius:10px;padding:12px 16px;margin:4px 0 12px;text-align:${isRtl ? 'right' : 'left'};">
            <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:6px;">
              <span>ðŸ“…</span>
              <div>
                <div style="font-size:0.66rem;font-weight:700;color:#8a6000;text-transform:uppercase;">${isFr2 ? 'Date' : 'Ø§Ù„ØªØ§Ø±ÙŠØ®'}</div>
            ...
            <div style="display:flex;align-items:flex-start;gap:8px;">
              <span>ðŸ“</span>
              <div>
                <div style="font-size:0.66rem;font-weight:700;color:#8a6000;text-transform:uppercase;">${isFr2 ? 'Lieu' : 'Ø§Ù„Ù…ÙƒØ§Ù†'}</div>
                <div style="font-size:0.92rem;font-weight:600;color:#3d2600;">${isFr2 ? 'Palais des Fetes â€” Teboulba, Monastir' : 'Ù‚ØµØ± Ø§Ù„Ø£ÙØ±Ø§Ø­ â€” Ø·Ø¨Ù„Ø¨Ø©ØŒ Ø§Ù„Ù…Ù†Ø³ØªÙŠØ±'}</div>
              </div>
            </div>
          </div>

          <div style="color:#c9a84c;letter-spacing:4px;font-size:0.65rem;margin:4px 0 10px;">â€” âœ¦ â€”</div>
          <div style="font-size:0.82rem;color:#7a6035;font-style:italic;line-height:1.55;margin-bottom:14px;">
            ${isFr2 ? "Nous vous remercions pour votre presence â¤" : "Ù†Ø´ÙƒØ±ÙƒÙ… Ø¹Ù„Ù‰ Ø­Ø¶ÙˆØ±ÙƒÙ… ÙˆÙ…Ø­Ø¨ØªÙƒÙ… â¤"}
          </div>

          <!-- Bottom seal -->
          <div style="text-align:center;padding-bottom:14px;">
            <div style="display:inline-flex;flex-direction:column;align-items:center;">
              <div style="width:60px;height:60px;border-radius:50%;background:radial-gradient(circle at 38% 35%,#f5e190 0%,#d4a830 25%,#b8860b 50%,#7a5300 75%,#3d2600 100%);border:2px solid #c9a84c;box-shadow:0 0 0 3px rgba(201,168,76,0.18),0 8px 22px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;font-size:1.3rem;color:rgba(255,252,220,0.9);">âœ¦</div>
              <div style="margin-top:5px;font-size:0.62rem;color:#8a6000;letter-spacing:2px;text-transform:uppercase;">${isFr2 ? 'Sceau Royal' : 'Ø§Ù„Ø®ØªÙ… Ø§Ù„Ù…Ù„ÙƒÙŠ'}</div>
            </div>
          </div>
        </div>
        ...
        <!-- Vignette -->
        <div style="position:absolute;inset:0;background:radial-gradient(ellipse at center,transparent 50%,rgba(30,15,5,0.18) 100%);border-radius:4px;pointer-events:none;"></div>
      </div>

      <!-- Polaroid caption -->
      <div style="padding:10px 16px 6px;text-align:center;font-family:'Courier New',monospace;direction:ltr;">
        <div style="font-size:0.72rem;color:#5a4020;letter-spacing:1.5px;opacity:0.75;">
          âœ¦ ${groom} &amp; ${bride} Â· ${dayNum} ${monthName} ${year} Â· ${isFr2 ? 'Souvenir' : 'ØªØ°ÙƒØ§Ø±'} âœ¦
        </div>
      </div>
    </div>
  `;
